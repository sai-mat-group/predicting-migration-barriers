...
def collate_line_graph(samples):
        ...
        #)
        return (
        (batched_graph1, batched_line_graph1),
        (batched_graph2, batched_line_graph2),
        torch.tensor(labels),
    )

### Parameters that we need to set
data = 'MPT-BOTH-SN-Em-train.json'
epochs = 60
...
checkpoint_fp = 'checkpoint_441.pt' # The checkpoint of the general model to load up initially
...
layers_to_copy = ['atom_embedding', 'edge_embedding', 'angle_embedding', 'alignn_layers', 'gcn_layers', 'readout','conv_to_fc','conv_to_fc_activation','fc_hp','fc_activation','fc_out']
...
model = ALIGNN_ft(n_outputs=n_outputs_new,
        print_outputs=print_outputs, n_hidden=n_hidden)
model.to(device)
model.load_state_dict(torch.load(checkpoint_fp, map_location=torch.device(device))['model'])
