...
data = 'MPT-SINGLE-BOTH-SN-Em-test.json'
epochs = 1
...
preds_file = './SINGLE-PATH-TEST.csv'
...
for i in tqdm(range(len(all_data))):
    val_results.append((model([(all_data.graphs1[i].to(device), all_data.line_graphs1[i].to(device)),(all_data.graphs2[i].to(device), all_data.line_graphs2[i].to(device))]).item(), 
            all_data[i][-1].item()))
