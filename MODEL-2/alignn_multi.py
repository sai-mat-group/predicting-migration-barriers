...
class ALIGNN_ft(nn.Module):
    ...
    def __init__(self, config: ALIGNNConfig = ALIGNNConfig(name="alignn"), n_outputs=6, print_outputs=False, 
        ...
        self.alignn_layers_1 = nn.ModuleList(
            [
                ALIGNNConv(config.hidden_features, config.hidden_features,)
                for idx in range(1)
            ]
        )
        self.gcn_layers_1 = nn.ModuleList(
            [
                EdgeGatedGraphConv(
                    config.hidden_features, config.hidden_features
                )
    ...
    def forward(
        ...
        #print(g1_lg1)
        #print(g2_lg2)
        ####Get layers 1-6 for second input for DELTA#############
        if len(self.alignn_layers) > 0:
            g2, lg2 = g2_lg2
            lg2 = lg2.local_var()
        ...
        # ALIGNN updates: update node, edge, triplet features
        for alignn_layer in self.alignn_layers_1:
            x2, y2, z2 = alignn_layer(g2, lg2, x2, y2, z2)

        # gated GCN updates: update node, edge features
        for gcn_layer in self.gcn_layers_1:
            x2, y2 = gcn_layer(g2, x2, y2)

        # norm-activation-pool-classify
        h2_del = self.readout(g2, x2)

        ####Get layers 1-6 for first input for DELTA################
        if len(self.alignn_layers) > 0:
            g1, lg1 = g1_lg1
            lg1 = lg1.local_var()
            # angle features (fixed)
