    ...
    def atom_dgl_multigraph(
        ...
        if neighbor_strategy == "k-nearest":
            ...
            u, v, r = build_undirected_edgedata(atoms, edges)
                        ###########IMAGE 1################
            #Collecting relevant edges
            edges_sub = nearest_neighbor_edges(atoms=atoms, cutoff=3.0, max_neighbors=6)
            #filtered_edges1 = {key: val for key, val in edges_sub.items() if key[0] == 0 or key[1] == 0}
            filtered_edges1 = {key: val for key, val in edges_sub.items() if key[0] < 5 or key[1] < 5}

            unique_nodes1 = sorted(set([idx for edge in filtered_edges1.keys() for idx in edge]))

            # Collecting only relevant atoms
            sub_cart_coords = [atoms.cart_coords[i] for i in unique_nodes1]
            sub_frac_coords = [atoms.frac_coords[i] for i in unique_nodes1]
            sub_elements = [atoms.elements[i] for i in unique_nodes1]
            sub_atoms = Atoms(lattice_mat=atoms.lattice_mat,
                                elements=sub_elements,
                                coords=sub_frac_coords,
                                cartesian=False,)
            node_map1 = {old_idx: new_idx for new_idx, old_idx in enumerate(unique_nodes1)}
            remapped_edges1 = {(node_map1[src], node_map1[dst]): val for (src, dst), val in filtered_edges1.items()}
            u1, v1, r1 = build_undirected_edgedata(sub_atoms, remapped_edges1)
            g1 = dgl.graph((u1, v1))
            g1.edata["r"] = r1  # Assign edge features

            sps_features1 = []
            for ii, s in enumerate(sub_atoms.elements):
                feat = list(get_node_attributes(s, atom_features=atom_features))
                sps_features1.append(feat)
            sps_features1 = np.array(sps_features1)
            node_features1 = torch.tensor(sps_features1).type(
                torch.get_default_dtype()
            )
            g1.ndata["atom_features"] = node_features1
            g1.ndata["coords"] = torch.tensor(sub_atoms.cart_coords)
...
def prepare_line_graph_batch(
    batch: Tuple[
        Tuple[Tuple[dgl.DGLGraph, dgl.DGLGraph], Tuple[dgl.DGLGraph, dgl.DGLGraph]],
        torch.Tensor,
    ],
    device=None,
    non_blocking=False,
    ...
    Note: the batch is a nested tuple, with the graph and line graph together
    """
    (g1, lg1), (g2, lg2), t = batch 

    batch = (
        (
            (g1.to(device, non_blocking=non_blocking),
             lg1.to(device, non_blocking=non_blocking)),
            (g2.to(device, non_blocking=non_blocking),
             lg2.to(device, non_blocking=non_blocking)),
        ),
        t.to(device, non_blocking=non_blocking),
    )
    return batch
...
class StructureDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        self,
        df: pd.DataFrame,
        graphs: List[Sequence[dgl.DGLGraph]],
        target: str,
        target_atomwise="",
        target_grad="",
        target_stress="",
        ...
        self.df = df
        #self.graphs = graphs
        self.graphs1 = graphs[0]##### Change made #####
        self.graphs2 = graphs[1]##### Change made #####
        self.target = target
        ...
        self.labels = [(self.df[target][ii]) for ii in range(len(df[target]))]
        if len(self.has_prop) > 0:
            self.has_prop = [(self.df['has_prop'][ii]) for ii in range(len(df[target]))]
        ...
        for i, g1 in enumerate(graphs[0]):
            z = g1.ndata.pop("atom_features")
            g1.ndata["atomic_number"] = z
            z = z.type(torch.IntTensor).squeeze()
            f = torch.tensor(features[z]).type(torch.FloatTensor)
            if g1.num_nodes() == 1:
                f = f.unsqueeze(0)
            g1.ndata["atom_features"] = f
            if (
                self.target_atomwise is not None and self.target_atomwise != ""
            ):  # and "" not in self.target_atomwise:
                g1.ndata[self.target_atomwise] = self.labels_atomwise[i]
            if (
                self.target_grad is not None and self.target_grad != ""
            ):  # and "" not in  self.target_grad:
                g1.ndata[self.target_grad] = self.labels_grad[i]
            if (
                self.target_stress is not None and self.target_stress != ""
            ):  # and "" not in  self.target_stress:
                # print(
                ...
                # )
                g1.ndata[self.target_stress] = torch.tensor(
                    [self.labels_stress[i] for ii in range(len(z))]
                ).type(torch.get_default_dtype())
        ##### RD: Change made ##### 
        for i, g2 in enumerate(graphs[1]):
            z = g2.ndata.pop("atom_features")
            g2.ndata["atomic_number"] = z
            z = z.type(torch.IntTensor).squeeze()
            f = torch.tensor(features[z]).type(torch.FloatTensor)
            if g2.num_nodes() == 1:
                f = f.unsqueeze(0)
            g2.ndata["atom_features"] = f
            if (
                self.target_atomwise is not None and self.target_atomwise != ""
            ):  # and "" not in self.target_atomwise:
                g2.ndata[self.target_atomwise] = self.labels_atomwise[i]
            if (
                self.target_grad is not None and self.target_grad != ""
            ):  # and "" not in  self.target_grad:
                g2.ndata[self.target_grad] = self.labels_grad[i]
            if (
                self.target_stress is not None and self.target_stress != ""
            ):  # and "" not in  self.target_stress:
                # print(
                ...
                #    [self.labels_stress[i] for ii in range(len(z))],
                # )
                g2.ndata[self.target_stress] = torch.tensor(
                    [self.labels_stress[i] for ii in range(len(z))]
                ).type(torch.get_default_dtype())
        ##### RD: Change made #####
        ...
        if line_graph:
            ...
            self.line_graphs1 = []
            for g in tqdm(graphs[0]):
                lg = g.line_graph(shared=True)
                lg.apply_edges(compute_bond_cosines)
                self.line_graphs1.append(lg)
    ...
    def __getitem__(self, idx):
        ...
        g1 = self.graphs1[idx]  # Get graph from the first set
        g2 = self.graphs2[idx]  # Get graph from the second set
        label = self.labels[idx]

        if self.transform:
            g1 = self.transform(g1)  # Apply transform to the first graph 
            g2 = self.transform(g2)

        if self.line_graph:
            if self.has_prop:
                hp = self.has_prop[idx]
                return (g1, g2), (self.line_graphs1[idx], self.line_graphs2[idx]),hp, label
            else:
                return (g1, g2), (self.line_graphs1[idx], self.line_graphs2[idx]), label
    ...
    def collate(
        samples: List[Tuple[Tuple[dgl.DGLGraph, dgl.DGLGraph], torch.Tensor]]
    ) -> Tuple[
    ...
    #):
        ...
        graphs1, graphs2, line_graphs1, line_graphs2, labels = (
            [],
            [],
            [],
            [],
        ...
        for (g1, g2), (lg1, lg2), label in samples:
            graphs1.append(g1)
            graphs2.append(g2)
            line_graphs1.append(lg1)
            line_graphs2.append(lg2)
            labels.append(label)

        batched_graph1 = dgl.batch(graphs1)
        batched_graph2 = dgl.batch(graphs2)
        batched_line_graph1 = dgl.batch(line_graphs1)
        batched_line_graph2 = dgl.batch(line_graphs2)

        if len(labels[0].size()) > 0:
            return (
            (batched_graph1, batched_graph2),
            (batched_line_graph1, batched_line_graph2),
            torch.stack(labels),
        )
        else:
            return (
