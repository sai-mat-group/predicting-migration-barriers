...
learning_rate = 1e-3 
prop_indices = [0, 1, 2, 4, 5, 6]  # This is the index of the property - taken from the OH column
n_early_stopping = 30
...
for i in tqdm(range(len(test_data))):
    test_results.append((model((test_data.graphs[i], test_data.line_graphs[i], test_data.has_prop[i]))[5].item(), test_data[i][-1][5]))

np.savetxt('frm-e.csv', test_results) 
