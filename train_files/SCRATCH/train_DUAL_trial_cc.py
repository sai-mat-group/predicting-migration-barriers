...
from alignn.data_dual import get_train_val_loaders
from alignn.config import TrainingConfig
from alignn.models.alignn_dual_cc import ALIGNN
...
def prepare_batch(batch, device=None, non_blocking=False):
    """Prepare batch for training, grouping (g1, lg1) and (g2, lg2). Store the grouped data 
    in the format that Alignn model will take as input
    """
    g1 = batch[0][0].to(device=device, non_blocking=non_blocking)
    g2 = batch[0][1].to(device=device, non_blocking=non_blocking)
    lg1 = batch[1][0].to(device=device, non_blocking=non_blocking)
    lg2 = batch[1][1].to(device=device, non_blocking=non_blocking)
    target = batch[2].to(device=device, non_blocking=non_blocking)
    return [(g1, lg1), (g2, lg2)], target
...
def train_dgl(
        ...
        if len(test_loader) > 0:
            with torch.no_grad():
                ...
                for dat, id in zip(test_loader, ids):
                    ...
                    lg2 = dat[1][1]
                    target = dat[2]
                    out_data = net([(g1.to(device), lg1.to(device)), (g2.to(device), lg2.to(device))])
                    out_data = out_data.cpu().numpy().tolist()
                    if config.standard_scalar_and_pca:
                        sc = pk.load(
    ...
    #                lg2 = dat[1][1]
    #                target = dat[2]
    #                out_data, h = net([(g1.to(device), lg1.to(device)), (g2.to(device), lg2.to(device))])
    #                out_data = out_data.cpu().numpy().tolist()
    #                target = target.cpu().numpy().flatten().tolist()
    #                h = h.cpu().numpy().tolist()
    #                if len(target) == 1:
    #                    target = target[0]
    #                new_entry = {"jid": id, "embedding":h,"target": target}
    #                emb.append(new_entry)
    #        with open(os.path.join(config.output_dir, "embedding_target.json"),"w",) as outfile:
    #            json.dump(emb, outfile, indent=4)
